    ...
    <div>
        <el-dialog :visible.sync="houseTypeDialogVisible" :title="propData.title +'-房型'" class="right-dialog" custom-class="left" width="80%">
            <div class="filter-container">
                ...
                <el-button v-if="btnAry.findIndex(x => x.id == '71')!=-1" class="filter-item" type="primary" size="mini" icon="el-icon-plus" @click="handleCreate">添加</el-button>
                <el-button v-if="btnAry.findIndex(x => x.id == '72')!=-1" class="filter-item" type="danger" size="mini" icon="el-icon-close" @click="delMore">删除</el-button>
            ...
            <el-table-column label="操作" align="center" width="250" class-name="fixed-width">
                ...
                <el-button  v-if="btnAry.findIndex(x => x.id == '668')!=-1" type="primary" size="mini" @click="handleStatusChange(row)">房态</el-button>
                <el-button v-if="btnAry.findIndex(x => x.id == '73')!=-1" type="primary" size="mini" icon="el-icon-edit" @click="handleUpdate(row)">
                    编辑
                </el-button>
                <el-button v-if="btnAry.findIndex(x => x.id == '72')!=-1" size="mini" type="danger" icon="el-icon-close" @click="handleDelete(row,$index)">
                    删除
                </el-button>
                </template>
            </el-table-column>
                    ...
                    <el-form ref="editForm2" :rules="rules" :model="editForm" label-position="left" label-width="120px" style="margin-bottom:50px">
                        ...
                        <div v-for="(item,index) in roomAttrList" :key="index">
                            <el-form-item v-if="item.output == 'input'" :label="item.attr_name">
                                <el-input v-model="item.value" style="display: inline-block;width:150px"></el-input>
                            </el-form-item>
...
getRoomPriceAdd,getRoomPriceEdit,getRoomPriceDel} from '@/api/hotel-manage/house-type'
import { getHotelService} from '@/api/hotel-manage/hotel-list'
import Pagination from '@/components/Pagination'
import ImgGallery from '@/components/ImgGallery'
import Tinymce from '@/components/Tinymce'
    export default {
        components: { Pagination,ImgGallery,Tinymce},
        data() {
            return {
        ...
        methods: {
            ...
            // 点击修改状态
            handleStatus(row) {
                if (this.btnAry.findIndex(x => x.id == '669') == -1) {
                this.$message({
                    type: 'warning',
                    message: '您没有权限修改'
                })
