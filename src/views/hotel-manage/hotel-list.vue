<template>
  <div>
    ...
    <div class="filter-container">
      ...
      <el-button v-if="btnAry.findIndex(x => x.id == '65')!=-1" class="filter-item" type="primary" size="mini" icon="el-icon-plus" @click="handleCreate">添加</el-button>
      <!-- <el-button v-if="btnAry.findIndex(x => x.id == '66')!=-1" class="filter-item" type="danger" size="mini" icon="el-icon-close" @click="delMore">删除</el-button> -->
      <el-button v-if="btnAry.findIndex(x => x.id == '67')!=-1" class="filter-item" type="primary" size="mini" :loading="exportAllloading" @click="exportTable('1')">全部导出</el-button>
      <el-button v-if="btnAry.findIndex(x => x.id == '68')!=-1" class="filter-item" type="primary" size="mini" @click="exportTable('2')">按页导出</el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="所在城市" align="left">
        <template slot-scope="{row}">
      ...
      <el-table-column label="酒店星级" align="center"  width="100">
        <template slot-scope="{row}">
      ...
      <el-table-column label="特价酒店" align="center">
        <template slot-scope="{row}">
          <span v-if="row.is_discount == 0">否</span>
          <span v-if="row.is_discount == 1">是</span>
        </template>
      </el-table-column>
       <el-table-column label="运营人" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="200" class-name="fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="btnAry.findIndex(x => x.id == '69')!=-1" type="primary" size="mini" icon="el-icon-edit" @click="handleUpdate(row)">
            编辑
          </el-button>
          <el-dropdown  trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="btnAry.findIndex(x => x.id == '70')!=-1"  @click.native="handleHouseType(row)">房型管理</el-dropdown-item>
              <el-dropdown-item v-if="btnAry.findIndex(x => x.id == '74')!=-1" @click.native="handleHousePhoto(row)">相册管理</el-dropdown-item>
              <el-dropdown-item v-if="btnAry.findIndex(x => x.id == '75')!=-1"  @click.native="handleHouseConfiguration(row)">酒店配置</el-dropdown-item>
              <el-dropdown-item v-if="btnAry.findIndex(x => x.id == '134')!=-1"  @click.native="handleGive(row)">赠送管理</el-dropdown-item>
              <el-dropdown-item   @click.native="handleAccountClick(row)">员工列表</el-dropdown-item>
              <el-dropdown-item  @click.native="handleQRcode(row)">酒店二维码</el-dropdown-item>
              <el-dropdown-item v-if="btnAry.findIndex(x => x.id == '136')!=-1" @click.native="handleCodeClick(row)">安全码</el-dropdown-item>
              <el-dropdown-item v-if="btnAry.findIndex(x => x.id == '66')!=-1" @click.native="handleDelete(row)">删除酒店</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        ...
        <el-tabs v-model="activeName" >
            <el-tab-pane label="基础信息" name="first">
                <el-form ref="editForm1" :rules="rules" :model="editForm" label-position="left" label-width="100px" style="width:80%;margin-bottom:50px">
                    ...
                    <!-- <el-form-item label="酒店运营人员" prop="type">
            ...
            <el-tab-pane label="酒店图片" name="second">
                <el-form ref="editForm2" :rules="rules" :model="editForm" label-position="left" label-width="100px" style="width:80%;margin-bottom:50px">
            ...
            <el-tab-pane label="周边景点" name="fifth">
               <tinymce ref="tinymce2"  v-model="editForm.around_spots"  :height="250" style="min-width:600px;width:80%"/>
            </el-tab-pane>
    ...
    <!-- 修改操作码 -->
    <setsecurity-code ref="setsecurityCodeChild" />
    <!-- 酒店二维码 -->
...
<script>
import { getHotelList,  getHotelAdd,  getHotelEdit,  getHotelDel,getHotelRank,getHotelCate,getHotelService,getHotelAdmin,getHotelExportExcel } from '@/api/hotel-manage/hotel-list'
...
import SetsecurityCode from '../security-code/setsecurity-code'
import HotelCode from './components/hotel-code'
import SecurityCode from '@/components/SecurityCode'
import mix from '@/mixs/mix'
export default {
  components: { Pagination,ImgGallery,Tinymce,BaiduMapmy,HouseType,HotelPhoto,HotelConfiguration,GiveScore,SetsecurityCode,HotelCode,SecurityCode },
  mixins: [mix],
  data() {
  ...
  mounted () {
    this.$store.dispatch('user/showBtn', { id: 20 }).then(res => {
      // console.log('res', res)
      this.btnAry = res.children
      if(this.btnAry.findIndex(x => x.id == '70')!=-1 || this.btnAry.findIndex(x => x.id == '74')!=-1
       || this.btnAry.findIndex(x => x.id == '75')!=-1 || this.btnAry.findIndex(x => x.id == '66')!=-1){
  ...
  methods: {
    ...
    //安全码
    handleCodeClick(row){
      this.$refs.setsecurityCodeChild.init(row)
    },
    // 点击修改状态
    handleStatus(row) {
    ...
    handleDelete(row, index) {
      this.$confirm('确认要删除吗', '提示', {
        ...
        .then(() => {
          this.$refs.securityCodeChild.init(row.id,'1')
    ...
    delMore() {
      ...
      this.$confirm('确认要删除吗', '提示', {
        ...
        .then(() => {
          ...
          this.$refs.securityCodeChild.init('','1')
