  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" >
      <!-- <el-table-column type="selection" width="55" /> -->
       ...
       <el-table-column label="酒店名称" align="left" width="200">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="180" class-name="fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="btnAry.findIndex(x => x.id == '114')!=-1" type="primary" size="mini" icon="el-icon-edit" @click="handleUpdate(row)">
            回复
          </el-button>
          <el-button v-if="btnAry.findIndex(x => x.id == '115')!=-1" type="danger" size="mini" icon="el-icon-close" @click.native="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  mounted () {
    this.getList();
    this.$store.dispatch('user/showBtn', { id: 37 }).then(res => {
      // console.log('res', res)
  ...
  methods: {
    ...
    // 点击修改状态
    handleStatus(row) {
      if (this.btnAry.findIndex(x => x.id == '116') == -1) {
        this.$message({
          type: 'warning',
          message: '您没有权限修改'
        })
