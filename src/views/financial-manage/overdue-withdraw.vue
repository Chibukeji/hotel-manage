<template>
  <div class="mixin-components-container">
        <div class="filter-container">
            ...
            <el-button v-if="btnAry.findIndex(x => x.id == '164')!=-1" class="filter-item" type="primary" size="mini" @click="exportTable('1')">全部导出</el-button>
            <el-button v-if="btnAry.findIndex(x => x.id == '163')!=-1" class="filter-item" type="primary" size="mini" @click="exportTable('2')">按页导出</el-button>
        </div>
        ...
        <aside>
          ...
          <span class="margin-right-20"> 
            <span>提现中金额:</span>
            <span style="color:red">{{totalData.cashing_amount}}</span>
            <span>元</span>
          </span>
          <span class="margin-right-20"> 
            <span>已提现金额:</span>
            <span style="color:red">{{totalData.cashed_amount}}</span>
            <span>元</span>
          </span>
          <span class="margin-right-20"> 
            <span>提现未通过金额:</span>
            <span style="color:red">{{totalData.failed_amount}}</span>
            <span>元</span>
          </span>
        </aside>
        <div>
          <el-table :data="list" border fit highlight-current-row style="width: 100%">
            ...
            <el-table-column label="结算日期" align="center" width="160px">
              <template slot-scope="{row}">
                <span>{{row.date}}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="返还积分" align="center" >
              <template slot-scope="{row}">
                <span>{{ row.score }}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" width="200">
              <template slot-scope="scope">
                ...
                <span v-else>
                  <el-button v-if="scope.row.status==0 && btnAry.findIndex(x => x.id == '157')!=-1" type="primary" size="mini" @click="cashhandle(scope.row)">提现</el-button>
                  <el-button v-if="btnAry.findIndex(x => x.id == '158')!=-1" type="primary" size="mini" @click="detailList(scope.row)">查看详情</el-button>
                </span>
              </template>
            </el-table-column>
          </el-table>
              ...
              <div class="left-box item-box">
                ...
                <span>
                  <div class="item-money first-item">
                    <span class="final-statement">￥{{billDetail.score}}</span>
                    <span class="final-statement"></span>
                  </div>
                  <div class="item-money">
                    <span>提现金额：</span>
                    <span>{{billDetail.score}}</span>
                  </div>
...
import {getWithdrawList,getWithdrawDetailList,getToWithdraw,orderWithdrawListExcel} from "@/api/financial-manage/overdue-withdraw";
...
export default {
  ...
  data() {
    return {
      ...
      btnAry:[],
      totalData:{
        cash_no_amount: 0,//未提现总额
        cashing_amount: 0,//提现中总额
        cashed_amount: 0,//已提现总额
        failed_amount: 0//未通过总额
      }
    };
  },
  mounted() {
    this.getList();
    this.$store.dispatch('user/showBtn', { id: 156 }).then(res => {
      // console.log('res', res)
