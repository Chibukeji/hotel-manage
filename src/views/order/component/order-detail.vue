...
<script>
import { getMemberOrderList,getMemberOrderChildList,getMemberOrderLogList,getStayPeopleList,getRefundOrderList } from '@/api/order/order-list'
...
export default {
  ...
  methods: {
    init(item) {
        ...
        if(this.detailItem.status != 4 && this.detailItem.status != 7 && this.detailItem.status != 8){
    ...
    getRefundOrder(){
      ...
      getRefundOrderList({order_id:this.detailItem.id}).then(res=>{
        this.refundOrderList = res.data.data.data
