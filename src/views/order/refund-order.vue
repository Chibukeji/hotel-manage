<template>
  <div>
    <div class="filter-container">
        ...
        <el-button v-if="btnAry.findIndex(x => x.id == '91')!=-1" class="filter-item" type="primary" size="mini" :loading="exportAllloading" @click="exportTable('1')">全部导出</el-button>
        <el-button v-if="btnAry.findIndex(x => x.id == '92')!=-1" class="filter-item" type="primary" size="mini" @click="exportTable('2')">按页导出</el-button>
     </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      <el-table-column label="订单号" align="center" width="200">
        <template slot-scope="{row}">
           <span v-if="btnAry.findIndex(x => x.id == '93')!=-1" style="color:#008ed8;cursor: pointer;"  @click="handleDetail(row,'none')">{{ row.order_sn }}</span>
      ...
      <el-table-column label="操作" align="center" width="160" class-name="fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" size="mini"  v-if="row.status == 0 && btnAry.findIndex(x => x.id == '94')!=-1" @click="handleRefund(row)">退款</el-button>
          <el-button v-if="btnAry.findIndex(x => x.id == '93')!=-1" type="primary" size="mini"  @click="handleDetail(row,'none')">订单详情</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getMerchantList} from '@/api/merchant-manage/merchant-list'
...
export default {
  ...
  mounted () {
    this.getList();
    this.$store.dispatch('user/showBtn', { id: 23 }).then(res => {
      // console.log('res', res)
